...
param_test1 = {
 'max_depth':[1,6],
 'min_child_weight':[.05,4],
 'objective':['reg:linear','binary:logistic']
}
print "Making GridSearchCV object"
gsearch1 = GridSearchCV(estimator = XGBClassifier( learning_rate =0.1, n_estimators=optimized_n_est, max_depth=6,
 min_child_weight=1, gamma=0, colsample_bytree=.5,
 objective= 'binary:logistic', nthread=5, scale_pos_weight=1, seed=0, verbose=10), 
 param_grid = param_test1, scoring='average_precision',n_jobs=1,iid=False, cv=5)
print "Fitting"
gsearch1.fit(X,y)

now = datetime.datetime.now()
if not path.exists("param_tuning_output"):
    makedirs("param_tuning_output")
file_name = "param_tuning_output/xgboost_param_tuning_"+str(now).replace(":","-")+".txt"
f = open(file_name,'w+')
for item in gsearch1.grid_scores_:
    print item
    f.write(str(item)+"\n")
f.write("\nBest parameters: "+str(gsearch1.best_params_) + "\n")
f.write("Best parameter score: " + str(gsearch1.best_score_))
f.close()
